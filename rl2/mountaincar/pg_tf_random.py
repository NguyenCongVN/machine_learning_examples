...
class PolicyModel:
    def __init__(self, ft, D, hidden_layer_sizes_mean=[], hidden_layer_sizes_var=[]):
        ...
        # final layer
        layer = HiddenLayer(M1, 1, lambda x: x, use_bias=False,
                            zeros=True)  # hidden layer is neuron because mean is unbounded
        self.mean_layers.append(layer)
        ...
        # final layer
        layer = HiddenLayer(M1, 1, tf.nn.softplus, use_bias=False,
                            zeros=False)  # hidden layer is softplus because variance must be greater 0
        self.var_layers.append(layer)
        ...
        norm = tfd.Normal(mean, std)
        self.predict_op = tf.clip_by_value(norm.sample(), -1, 1)  # if lower than -1 -> -1 , if greater than 1 -> 1
    ...
    def copy(self):  # climb hill related
        clone = PolicyModel(self.ft, self.D, self.hidden_layer_sizes_mean,
                            self.hidden_layer_sizes_mean)  # tạo ra các clone Model -> sử dụng để tạo các neighbor parameters
